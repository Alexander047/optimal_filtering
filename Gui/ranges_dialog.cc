...
RangesDialog::RangesDialog (const Vector2 &x, const Vector2 &y, QWidget *parent) : QDialog (parent)
{
    initControls (x, y);
    initPlotter();
    loadFonts();
...
void RangesDialog::initControls (const Vector2 &x, const Vector2 &y)
{
    ...
    connect (m_dsbXMin, SIGNAL (valueChanged (double) ), this, SLOT (xMinChanged (double) ));
    ...
    connect (m_dsbXMax, SIGNAL (valueChanged (double) ), this, SLOT (xMaxChanged (double) ));
    ...
    connect (m_dsbYMin, SIGNAL (valueChanged (double) ), this, SLOT (yMinChanged (double) ));
    ...
    connect (m_dsbYMax, SIGNAL (valueChanged (double) ), this, SLOT (yMaxChanged (double) ));

    m_buttonBox = new QDialogButtonBox;
    m_buttonBox->setOrientation (Qt::Horizontal);
    m_buttonBox->setStandardButtons (QDialogButtonBox::Ok | QDialogButtonBox::Cancel);
    connect (m_buttonBox, SIGNAL (accepted()), this, SLOT (acceptedClicked()));
    connect (m_buttonBox, SIGNAL (rejected()), this, SLOT (rejectedClicked()));
}
...
void RangesDialog::loadFonts()
{
    ...
    int dsbHeight        = 1.67 * metric->height();
    ...
    this->setMinimumWidth (4.1 * dsbWidth);
    this->setMinimumHeight (4.1 * dsbWidth);
}
...
void RangesDialog::yMaxChanged (double yMax)
{
...
void RangesDialog::yMinChanged (double yMin)
{
...
void RangesDialog::xMaxChanged (double xMax)
{
...
void RangesDialog::xMinChanged (double xMin)
{
...
void RangesDialog::acceptedClicked()
{
    Vector2 x, y;
...
void RangesDialog::rejectedClicked()
{
    close();
}
