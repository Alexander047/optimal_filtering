...
enum AntialiasedElement {
    ...
    ,
    aePlottables = 0x0020  ///< <tt>0x0020</tt> Main lines of plottables (excluding error bars, see
                           ///element \ref aeErrorBars)
    ,
...
enum PlottingHint {
    ...
    ,
    phForceRepaint = 0x002  ///< <tt>0x002</tt> causes an immediate repaint() instead of a soft
...
public:
    ...
    */
    enum LabelType {
        ltNumber  ///< Tick coordinate is regarded as normal number and will be displayed as such.
                  ///(see \ref setNumberFormat)
        ,
        ltDateTime  ///< Tick coordinate is regarded as a date/time (seconds since
                    ///1970-01-01T00:00:00 UTC) and will be displayed and formatted as such. (for
                    ///details, see \ref setDateTimeFormat)
    };
    Q_ENUMS (LabelType)
    /*!
    ...
    */
    enum ScaleType {
        ...
        ,
        stLogarithmic  ///< Logarithmic scaling with correspondingly transformed plots and (major)
                       ///tick marks at every base power (see \ref setScaleLogBase).
    };
    Q_ENUMS (ScaleType)
    /*!
...
protected:
    /*!
      Represents negative and positive sign domain for passing to \ref getKeyRange and \ref
      getValueRange.
    */
    enum SignDomain {
...
public:
    ...
    */
    enum RefreshPriority {
        rpImmediate  ///< The QCustomPlot surface is immediately refreshed, by calling
                     ///QWidget::repaint() after the replot
        ,
        rpQueued  ///< Queues the refresh such that it is performed at a slightly delayed point in
                  ///time after the replot, by calling QWidget::update() after the replot
        ,
        rpHint  ///< Whether to use immediate repaint or queued update depends on whether the
                ///plotting hint \ref QCP::phForceRepaint is set, see \ref setPlottingHints.
    };
    ...
    bool savePdf (const QString &fileName, bool noCosmeticPen = false, int width = 0,
                  int height = 0, const QString &pdfCreator = QString(),
...
protected:
    ...
    int getRegion (double x, double y, double rectLeft, double rectTop, double rectRight,
    ...
    bool getTraverse (double prevKey, double prevValue, double key, double value, double rectLeft,
                      double rectTop, double rectRight, double rectBottom, QPointF &crossA,
                      QPointF &crossB) const;
    void getTraverseCornerPoints (int prevRegion, int currentRegion, double rectLeft,
...
protected:
    ...
    QLineF getRectClippedStraightLine (const QVector2D &point1, const QVector2D &vec,
